...
*/
void AnimatedImage::setPalette(ColorMapObject* colorMap, SDL_Surface* surface) {
...
*/
void AnimatedImage::setIndex(uint16_t index) {
...
*/
uint16_t AnimatedImage::getDelay(uint16_t index) {
	index %= frame_count;
...
*/
uint16_t AnimatedImage::getDelay() {
...
* index 			> Target frame index
*/
ExtensionBlock* AnimatedImage::getGraphicsBlock(uint16_t index) {
...
void AnimatedImage::prerender()
{
	for (int i = 0; i < this->frame_count; i++) {
		prepare(i);
		this->frames.push_back(this->texture_);
		this->texture_ = nullptr; //remove reference so next call to prepare doesn't free it
	}
	return;
}

/*COPY FROM IMAGE.CPP JUST FOR PREVENT ERROR*/
...
//AnimatedImage::AnimatedImage(SDL_Renderer* renderer, std::filesystem::path path) {
// 
// 
		...
		
{
	...
	this->texture_ = SDL_CreateTextureFromSurface(SDL::getRenderer(baseViewInfo.Monitor)/*this->renderer*/, this->surface);

	if (this->animated) 
	{
		if (this->prerendered) prerender();
		animationThread = std::thread(&AnimatedImage::animating, this);
	}
}
...
*/
void AnimatedImage::prepare(uint16_t index) {
	...
	SDL_DestroyTexture(this->texture_); //delete old texture
	this->texture_ = SDL_CreateTextureFromSurface(SDL::getRenderer(baseViewInfo.Monitor)/*this->renderer*/, this->surface);
...
*/
void AnimatedImage::prepare() {
	if (prerendered) {
		this->texture_ = frames[frame_index];
		this->ready = false;
	}
	else {
		prepare(frame_index);
...
/**   DISABLE IT SHOULD BE PLAY ALWAYS
